...
mod tests {
	...
	fn test_load_from_path_valid() {
		let temp_dir = TempDir::new().unwrap();
		let monitor_path = temp_dir.path().join("valid_monitor.json");
		// Test valid configuration
		let monitor = Monitor {
			name: "test_monitor".to_string(),
		...
		fs::write(&monitor_path, serde_json::to_string(&monitor).unwrap()).unwrap();

		let repository =
			MonitorRepository::<NetworkRepository, TriggerRepository>::new(None, None, None)
				.unwrap();
	...
	#[test]
	fn test_load_from_path_invalid_path() {
		let invalid_path = Path::new("/non/existent/path");
		let repository =
			MonitorRepository::<NetworkRepository, TriggerRepository>::new(None, None, None)
				.unwrap();
	...
	#[test]
	fn test_load_from_path_invalid_references() {
		let temp_dir = TempDir::new().unwrap();
		let monitor_path = temp_dir.path().join("invalid_references.json");
		// Test invalid references configuration
		let monitor = Monitor {
			name: "test_monitor".to_string(),
			networks: vec!["non_existent_network".to_string()],
			paused: false,
	...
	#[test]
	fn test_load_from_path_missing_path() {
		let repository =
			MonitorRepository::<NetworkRepository, TriggerRepository>::new(None, None, None)
				.unwrap();
		let result = repository.load_from_path(None, None, None);
