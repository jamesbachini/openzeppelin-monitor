...
You can test the monitor in two different ways:

1. **Run monitor for latest blocks on all networks configured for the monitor**:
+
[source,bash]
----
cargo run -- --monitorPath="config/monitor/evm_transfer_usdc.json"
----
+
This command will:
* Execute the monitor named "Large Transfer of USDC Token"
* Run it against all networks specified in the monitor configuration
* Process the latest block for each network

2. **Run monitor for a specific block number**:
+
[source,bash]
----
cargo run -- --monitorPath="config/monitor/evm_transfer_usdc.json" --network=ethereum_mainnet --block=12345678
----
+
This command will:
* Execute the monitor named "Large Transfer of USDC Token"
* Run it against the specified network (ethereum_mainnet in this example)
* Process the specific block number (12345678 in this example)

[NOTE]
====
When running with a specific block number, both `--network` and `--block` parameters are required.
====
