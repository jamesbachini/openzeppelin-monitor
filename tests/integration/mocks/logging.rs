...
/// This integration test simulates when the monitor.log file already exists and verify if the new
...
fn test_log_file_rolls_when_existing() {
	...
	let base_file = temp_log_dir.join("monitor.log");
	...
	// The rolled path should differ from the base file path and contain "monitor-".
	assert_ne!(
		rolled_path,
	...
	assert!(
		rolled_path.contains("monitor-"),
		"Expected rolled log file path to contain 'monitor-'"
	);
}
